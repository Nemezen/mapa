...
namespace sistema_coord
{
    public partial class Proveedores : Form
    {
        ...
        private void EliminarProveedorSeleccionado()
        {
            try
            {
                ...
                if (dataGridView1.SelectedRows.Count > 0)
                {
                    ...
                    Proveedor proveedorAEliminar = dbContext.Proveedores.FirstOrDefault(c => c.Id == idProveedor);

                    // Verificar si se encontró el proveedor
                    if (proveedorAEliminar != null)
                    {
                        // Eliminar el proveedor del contexto y guardar los cambios
                        dbContext.Proveedores.Remove(proveedorAEliminar);
                        dbContext.SaveChanges();

                        // Mostrar los proveedores actualizados en el DataGridView
                        mostrarProveedores();
                    ...
                    {
                        MessageBox.Show("No se encontró el proveedor seleccionado.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                else
                {
        ...
        private void EditarTodo(string nombre, string direccion, string colonia, double latitud, double longitud)
        {
            ...
            {
                // Obtener el Proveedor existente por el Id
                int id = Convert.ToInt32(idProducto);
                Proveedor proveedorExistente = dbContext.Proveedores.FirstOrDefault(c => c.Id == id);
            ...
            {
                MessageBox.Show("Debe seleccionar un proveedor para editar.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void GuardarTodo(string nombre, string direccion, string colonia, double latitud, double longitud, DateTimePicker date)
        {
            try
            {
                ...
                // Agregar el nuevo Proveedor al contexto y guardar los cambios
        ...
        private void mostrarProveedores()
        {
            ...
            List<Proveedor> proveedor = dbContext.Proveedores.ToList();
            // Asignar la lista de proveedores al DataSource del DataGridView
